  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${baseUrl}/auth/login`, {
        ...
        body: JSON.stringify({ username: hostelId, email, password }),
      });
      ...
      if (res.ok) {
        ...
        router.push('/hostel/dashboard'); // Redirect to Hostel Dashboard
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans">
      ...
      <div className="flex-grow flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
          <h2 className="text-3xl font-bold text-center text-blue-800 mb-6 font-serif">Hostel Login</h2>
          {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">{error}</div>}
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="hostelId">
                Hostel ID
              </label>
              <input
