...
const UniversityHeader: React.FC = () => {
  ...
  const isLoggedIn = username && universityId;
      ...
      <header className="bg-white/90 backdrop-blur-lg shadow-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex flex-grow items-center justify-center space-x-8 font-sans font-medium">
            ...
            <Link href="/university/dashboard" className="text-slate-700 hover:text-lime-600 transition-colors">Dashboard</Link>
            <div className="relative group">
              <button className="text-slate-700 hover:text-lime-600 transition-colors flex items-center">
                <span>Register</span>
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 bg-white rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto z-50">
                <Link href="/university/register/hostel" className="block px-4 py-2 text-slate-700 hover:bg-lime-50">Register Hostels</Link>
                <Link href="/university/register/student" className="block px-4 py-2 text-slate-700 hover:bg-lime-50">Register Students</Link>
              </div>
            </div>
            ...
            {isLoggedIn ? (
              <>
                <div className="text-slate-700 font-semibold text-sm">
                  <p>{username}</p>
                  <p className="text-xs text-slate-800"><b>University ID:</b> {universityId}</p>
                </div>
                <Button
              ...
              <>
                <Link href="/login"><Button className="bg-transparent text-slate-700 hover:bg-slate-100">Log In</Button></Link>
                <Link href="/register"><Button className="bg-gradient-to-r from-lime-500 to-sky-500 text-white hover:from-lime-600 hover:to-sky-600 shadow-md hover:shadow-lg focus:ring-lime-400/50">Sign Up</Button></Link>
              </>
            )}
          </div>
      ...
      <div className={`fixed inset-0 z-50 transition-transform transform ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
        ...
        <div className="relative w-80 h-full bg-white ml-auto p-6 shadow-xl">
          ...
          <nav className="flex flex-col space-y-5 font-sans font-medium text-lg">
            ...
            <Link href="/university/dashboard" className="text-slate-800 hover:text-lime-600">Dashboard</Link>
            <Link href="/university/register/student" className="text-slate-800 hover:text-lime-600">Create Students</Link>
            <Link href="/university/register/hostel" className="text-slate-800 hover:text-lime-600">Create Hostels</Link>
          </nav>
          <div className="mt-8 pt-6 border-t border-slate-200 space-y-4">
            {isLoggedIn ? (
              <Button onClick={handleLogout} className="w-full bg-red-500 text-white hover:bg-red-600">Logout</Button>
            ) : (
              <>
